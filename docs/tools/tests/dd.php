	...
	<ul style="position: absolute; top: 10px; left: 5px" id="q">
		<?php for ($i=0; $i < 10; $i++): ?>
			<?php if ($row['choice_'. $i] != ''): ?>
				...
				<?php echo AT_print($row['choice_'.$i], 'tests_questions.choice_'.$i); ?></li>
			<?php endif; ?>
		<?php endfor; ?>
	</ul>
	...
	<ol style="position: absolute; list-style-type: upper-alpha; top: 10px; left: 310px" id="a">
		<?php for ($i=0; $i < 10; $i++): ?>
			<?php if ($row['option_'. $i] != ''): ?>
				<li class="answer" id="a<?php echo $i; ?>" value="<?php echo $i; ?>"><?php echo $_letters[$i]; ?>. <?php echo AT_print($row['option_'.$i], 'tests_questions.option_'.$i); ?></li>
			<?php endif; ?>
		<?php endfor; ?>
	</ol>
</form>
