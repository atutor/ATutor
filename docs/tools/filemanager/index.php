...
if ($_GET['frame']) {
	...
	echo '</p><br /><form name="checkform" action="tools/filemanager/file_manager_delete.php" method="post"><table cellspacing="1" cellpadding="0" border="0" class="bodyline" summary="" align="center">';
	echo '<input type="hidden" name="pathext" value ="'.$pathext.'" />'."\n";
	/* buttons */
	...
	if ($_GET['frame'] != 1) {
		...
		echo '<tr>'.$rowline.'</td></tr>'."\n";
		/* buttons */
		echo '<tr> <td colspan="6"> <input type="submit" name="newfile" value="New"><input type="submit" name="editfile" value='._AT('edit').'>&nbsp;';
		echo '<input type="submit" name="copyfile" value="Copy"><input type="submit" name="renamefile" value='._AT('rename').'>';
		echo '<input type="submit" name="deletefiles" value='._AT('delete').'><font face=arial size=-2>'._AT('checked_files').'</font>&nbsp;&nbsp;<nobr><input type="submit" name="movefilesub" value='._AT('move').'><input type="submit" name="copyfilesub" value="Copy"><font face=arial size=-2>'._AT('files_to_subdirectory').'</font></nobr></td></tr>';
