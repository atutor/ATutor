...
foreach($all_comments as $comment){
	if (isset($comment['photo_id'])){
		$list[] = _AT('comment').': <a href="'.$_base_href.AT_PA_BASENAME.'photo.php?aid='.$comment['album_id'].SEP.'pid='.$comment['photo_id'].'">'.htmlentities_utf8($comment['comment']).'</a>';
	} elseif (isset($comment['album_id'])){
		$list[] = _AT('comment').': <a href="'.$_base_href.AT_PA_BASENAME.'albums.php?id='.$comment['album_id'].'">'.htmlentities_utf8($comment['comment']).'</a>';
	}
