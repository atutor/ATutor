...
<form action="<?php echo $_SERVER['PHP_SELF'];?>" name="create_album" method="post">
	...
	<div class="row">
		...
		<label for="my_album"><?php echo _AT('my_albums'); ?><label><input type="radio" name="album_type" id="my_album" value="1" <?php echo ($this->album_info['type_id']==AT_PA_TYPE_MY_ALBUM)?' checked="checked"':'';?>/>
		<label for="course_album"><?php echo _AT('course_albums'); ?></label><input type="radio" name="album_type" id="course_album" value="2" <?php echo ($this->album_info['type_id']==AT_PA_TYPE_COURSE_ALBUM)?' checked="checked"':'';?>/>
	</div>
	<?php endif; ?>
	<div class="row">
