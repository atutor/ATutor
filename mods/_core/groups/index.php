...
/************************************************************************/
// $Id: index.php 10197 2010-09-16 16:18:25Z greg $
	...
	<?php do { ?>
		...
		<tr onmousedown="document.form['g<?php echo $row['type_id']; ?>'].checked = true; rowselect(this);" id="r_<?php echo $row['type_id']; ?>">
			<th>
				...
				<label for="g<?php echo $row['type_id']; ?>"><?php echo htmlentities_utf8($row['title']); ?></label> (<?php echo $num_groups.' '._AT('groups'); ?>)</td>
			</th>
		</tr>
		<?php if ($num_groups) : ?>
			<?php while ($group_row = mysql_fetch_assoc($group_result)): ?>
				...
				<tr onmousedown="document.form['g<?php echo $row['type_id'].'_'.$group_row['group_id']; ?>'].checked = true; rowselect(this);" id="r_<?php echo $row['type_id'].'_'.$group_row['group_id']; ?>">
					<td class="indent"><input type="radio" id="g<?php echo $row['type_id'].'_'.$group_row['group_id']; ?>" name="id" value="<?php echo $row['type_id'].'_'.$group_row['group_id']; ?>" /> <label for="g<?php echo $row['type_id'].'_'.$group_row['group_id']; ?>"><?php echo htmlentities_utf8($group_row['title']); ?></label> (<?php echo $group_cnt['cnt'].' '._AT('members'); ?>)</td>
				</tr>
			<?php endwhile; ?>
		<?php else: ?>
			<tr>
