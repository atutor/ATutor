...
class time {
	...
	function __construct($t, $milli=true) {
		if ($milli) {
			$this->ms = $t;
			$this->formatted = $this->timeFormatted($t);
		} else {
			$this->ms = $this->timeMilli($t);
			$this->formatted = $t;
		}
	}
	...
	/*
	 * Functions written by Antonio for the conversion service
	 */
	 ...
	 */
	static public function timeFormatted($miliSecTime)
	{
		$qtTimeString = '';
		//  milliseconds holder 
	...
	/**
	 * Converts QT time to milliseconds format (Accepted by SAMI 1.0 and other CCformats)
	 * @return int $samiTime Time in miliseconds format; 1000 = 1. sec
	 * @param String $qtTime QT time Format; (e.g. "00:01:10.280")
	 */
	static public function timeMilli($qtTime)
	{
	    ...
	    $qtTimeParts = explode(':',$qtTime); // split QT time mark into an array
