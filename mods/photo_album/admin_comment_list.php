...
/**
 * @desc	This file generates the administrator comment list page
...
if ($admin->isError()!=true){	//no error found, display the page
	$template=new HTML_Template_ITX("./Template");
	$template->loadTemplateFile("admin_comment_list.tpl.php");
	...
	switch ($admin->getVariable('mode')){
		...
		break;
		case APPROVED:
			$template->setVariable("TITLE", _AT('pa_title_administrator_approved_comment')." (".$course_name.")");
		break;
		case DISAPPROVED:
			$template->setVariable("TITLE", _AT('pa_title_administrator_disapproved_comment')." (".$course_name.")");
		break;
	}
	...
	if ($admin->getVariable('show_page_left_buttons')==true){
		...
		$template->setVariable("B_DATA", '<li><a href=\''.BASE_PATH.'admin_comment_list.php?current_page=1&amp;mode='.$admin->getVariable('mode').'\'><img src=\''.FIRST_PAGE_IMAGE.'\' alt=\''.$first_button.'\' width=\'30\' height=\'20\'/></a></li>');
		$template->parseCurrentBlock("B_DATA_PART");
		$template->setCurrentBlock("B_DATA_PART");
		$template->setVariable("B_DATA", '<li><a href=\''.BASE_PATH.'admin_comment_list.php?current_page='.($current-1).SEP.'mode='.$admin->getVariable('mode').'\'><img src=\''.PRE_IMAGE.'\' alt=\''.$previous_button.'\' width=\'30\' height=\'20\'/></a></li>');
		$template->parseCurrentBlock("B_DATA_PART");
	}
	...
	for ($i=$page_array['start']; $i<=$page_array['end']; $i++){
		if ($i==$current){
			...
			$template->setCurrentBlock("B_DATA_PART");
			$template->setVariable("B_DATA", '<li><a href=\''.BASE_PATH.'admin_comment_list.php?current_page='.$i.SEP.'mode='.$admin->getVariable('mode').'\'>'.$i.'</a></li>');
			$template->parseCurrentBlock("B_DATA_PART");
		}
	}
	...
	if ($admin->getVariable('show_page_right_buttons')==true){
		...
		$template->setCurrentBlock("B_DATA_PART");
		$template->setVariable("B_DATA", '<li><a href=\''.BASE_PATH.'admin_comment_list.php?current_page='.($current+1).SEP.'mode='.$admin->getVariable('mode').'\'><img src=\''.NEXT_IMAGE.'\' alt=\''.$next_button.'\' width=\'30\' height=\'20\'/></a></li>');
		$template->parseCurrentBlock("B_DATA_PART");
		$template->setCurrentBlock("B_DATA_PART");
		$template->setVariable("B_DATA", '<li><a href=\''.BASE_PATH.'admin_comment_list.php?current_page='.$page_array['last_page'].SEP.'mode='.$admin->getVariable('mode').'\'><img src=\''.LAST_PAGE_IMAGE.'\' alt=\''.$last_button.'\' width=\'30\' height=\'20\'/></a></li>');
		$template->parseCurrentBlock("B_DATA_PART");
	}
