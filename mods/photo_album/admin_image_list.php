...
/**
 * @desc	This file generates the administrator image list page
...
if ($admin->isError()!=true){	//no error was found, so display the page
	clear_temp_folder();
	$template=new HTML_Template_ITX("./Template");
	$template->loadTemplateFile("admin_image_list.tpl.php");
	$template->setVariable("ADMIN_NUMBER_OF_IMAGE", ADMIN_NUMBER_OF_IMAGE);
	$course_name=get_course_title($admin->getVariable('course_id'));	
	switch ($admin->getVariable('mode')){
		case POSTED_NEW:
			$template->setVariable("TITLE", _AT('pa_title_administrator_new_pic')." (".$course_name.")");
		break;
		case APPROVED:
			$template->setVariable("TITLE", _AT('pa_title_administrator_approved_pic')." (".$course_name.")");
		break;
		case DISAPPROVED:
			$template->setVariable("TITLE", _AT('pa_title_administrator_disapproved_pic')." (".$course_name.")");
		break;
	}
	...
	$template->setVariable("CHECK_MESSAGE", _AT('pa_tag_check_all_image'));
	...
	if ($admin->getVariable('show_page_left_buttons')==true){
		...
		$template->setVariable("B_DATA", '<li><a href=\''.BASE_PATH.'admin_image_list.php?current_page=1&amp;mode='.$admin->getVariable('mode').'\'><img src=\''.FIRST_PAGE_IMAGE.'\' alt=\''.$first_button.'\' width=\'30\' height=\'20\'/></a></li>');
		$template->parseCurrentBlock("B_DATA_PART");
		$template->setCurrentBlock("B_DATA_PART");
		$template->setVariable("B_DATA", '<li><a href=\''.BASE_PATH.'admin_image_list.php?current_page='.($current-1).SEP.'mode='.$admin->getVariable('mode').'\'><img src=\''.PRE_IMAGE.'\' alt=\''.$previous_button.'\' width=\'30\' height=\'20\'/></a></li>');
		$template->parseCurrentBlock("B_DATA_PART");
	}
	...
	for ($i=$page_array['start']; $i<=$page_array['end']; $i++){
		if ($i==$current){
			...
			$template->setCurrentBlock("B_DATA_PART");
			$template->setVariable("B_DATA", '<li><a href=\''.BASE_PATH.'admin_image_list.php?current_page='.$i.SEP.'mode='.$admin->getVariable('mode').'\'>'.$i.'</a></li>');
			$template->parseCurrentBlock("B_DATA_PART");
		}
	}
	...
	if ($admin->getVariable('show_page_right_buttons')==true){
		...
		$template->setCurrentBlock("B_DATA_PART");
		$template->setVariable("B_DATA", '<li><a href=\''.BASE_PATH.'admin_image_list.php?current_page='.($current+1).SEP.'mode='.$admin->getVariable('mode').'\'><img src=\''.NEXT_IMAGE.'\' alt=\''.$next_button.'\' width=\'30\' height=\'20\'/></a></li>');
		$template->parseCurrentBlock("B_DATA_PART");
		$template->setCurrentBlock("B_DATA_PART");
		$template->setVariable("B_DATA", '<li><a href=\''.BASE_PATH.'admin_image_list.php?current_page='.$page_array['last_page'].SEP.'mode='.$admin->getVariable('mode').'\'><img src=\''.LAST_PAGE_IMAGE.'\' alt=\''.$last_button.'\' width=\'30\' height=\'20\'/></a></li>');
		$template->parseCurrentBlock("B_DATA_PART");
	}
	...
	$_SESSION['pa']['administrator_mode']=true;
} else {
	$msg->addError('pa_obj_pa_admin_image');
	redirect('../../index.php'); 
}
?>
