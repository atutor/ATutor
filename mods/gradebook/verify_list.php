	...
	else 
	{
		...
		while ($data = fgetcsv($fp, 100000, ',')) {
			...
			if ($line_number > 1)
			{
				...
				if ($num_fields == 4)
				{
					$students[] = check_user_info(array('fname' => $data[0], 'lname' => $data[1], 'email' => $data[2], 'grade' => $data[3]));
				} 
...
if ($_POST['verify']) {
	...
	{
		$info = array('fname' => $_POST['fname'.$i], 'lname' => $_POST['lname'.$i], 'email' => $_POST['email'.$i], 'grade' => $_POST['grade'.$i], 'remove' => $_POST['remove'.$i]);
