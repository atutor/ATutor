...
?>
	<p><?php echo _AT('langs_on_this_system'); ?> </p>
		...
		foreach($available_languages as $key => $thislang){
			...
			if (($key != 'en') && !AT_DEVEL){
				echo ' <small>(<a href="users/admin/delete_lang.php?delete_lang='.$key.'">'._AT('remove').'</a>)</small>';
			}
			echo '</li>';
		}
		?>
...
<form name="form1" method="post" action="users/admin/import_lang.php" enctype="multipart/form-data" onsubmit="openWindow('<?php echo $_base_href; ?>tools/prog.php');">
